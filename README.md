...
**小事，顺手解决。** 手机优先、无需注册的小工具站，现有「随它转」和「单位比价」两个工具。
...
| 工具     | 地址                     | 功能                                                                      |
| -------- | ------------------------ | ------------------------------------------------------------------------- |
| 随它转   | `/tools/decision-wheel/` | 输入 2–24 个选项、等概率转盘、显示结果、自动暂存、保存/载入最多 30 组转盘 |
| 单位比价 | `/tools/unit-price/`     | 比较 2–6 个商品、重量/容量/数量换算、最低单价与差额、自动暂存             |
...
在选项区“生成转盘”下方点击“保存转盘”，弹出保存窗口。每次新建都会重新建议未使用的默认名称（转盘1、转盘2…），可直接保存，也可自定义。已载入一组时，可明确选择“存为新转盘”或“更新当前组”；更新模式保留原名，另存模式重新建议名称。

“我的转盘”通过按钮打开列表，手机上为底部面板，选中后自动关闭并载入，不占据日常转动页面。最多 30 组；删除及撤销删除在列表内完成。命名时会跳过已有名称，删除后空出的序号可以复用。

命名转盘使用独立存储键 `just-spin-presets-v1`，刷新和重开后仍保留；只存于当前浏览器，不跨设备同步，清除网站数据会丢失。保存失败会显示错误，不显示保存成功。
...
输入商品全部份数的实付总价、每份规格及份数，实时比较。支持克、千克、斤（固定为 500 克）、毫升、升和件。重量、容量、数量分别保存输入，不跨维度比较。价格可为零，份数必须为正整数；未填完或无效的商品不参与排名，页面会说明待填数量。

计算公式：`单位价格 = 实付总价 ÷ (每份规格 × 单位换算系数 × 份数)`。节省比例的分母是下一档较高单价，计算使用未舍入值，仅在显示时格式化。按相同品质和用途比较，工具只比较数量单价，不评判商品质量。输入存于当前浏览器的 `tiny-toolbox-unit-price-v1`，不会上传。
